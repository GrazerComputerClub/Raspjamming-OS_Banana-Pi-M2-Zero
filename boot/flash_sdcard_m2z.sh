...
#tar -xvpzf rootfs_kernel_4.17.y.tar.gz -C ./erootfs --numeric-ow
sync
sudo umount erootfs
rm -fR erootfs
sync
...
tar -xvpzf boot_kernel_4.17.y.tar.gz -C ./eboot  --numeric-ow
cp eboot/zImage_4.17.4-m2z eboot/zImage  
cp eboot/bpi-m2-zero_boot.scr eboot/boot.scr
cp eboot/bpi-m2-zero-v1.dtb_4.17.4-m2z eboot/bpi-m2-zero.dtb
sync
sudo umount eboot
rm -fR eboot
sync
